...
* Om Webappen
  ...
  - När containern får ett HTTP-request så letar den upp vilken servlet som ska hantera anropet genom att titta i appens "Deployment descriptor", där finns deklarationer av servlets samt url-mappings för dessa. Appens deployment descriptor utgörs av xml-configuration (T.ex. web.xml) samt annotationer på servletklasserna (@WebServlet). Raw-webapp använder sig bara av web.xml.
...
* Ett exempelanrop
  ...
  - När en jsp-sida efterfrågas så översätter JBoss innehåller i jsp-sidan till Servlet-kod, och kompilerar den sedan. D.v.s. en jsp som innehåller raden "Hello World" kommer att översättas till en Servlet vars doGet-metod innehåller "response.getWriter.println("Hello World");.
  - Vår jsp renderar outputen, till stor del genom getWriter.println():s, men även genom att anropa TagLib-kod (c:forEach till /c:forEach), som i sin tur bara är javakod.
